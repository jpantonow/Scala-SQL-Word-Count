...
class Insert_Book(path_to_text: String, path_to_database: String, book_name: String) extends Initialize(path_to_text: String, path_to_database: String){
...
class Register_Documents(path_to_text: String, path_to_database: String, book_name: String) extends Initialize(path_to_text: String, path_to_database: String){
    ...
    def length_25: Unit = {
        ...
        try{
            conn = DriverManager.getConnection(url)
            conn.setAutoCommit(false)
            var command = s"UPDATE OR IGNORE documents SET length_25 = (SELECT sum(len) " +
...
class Export_to_CSV (path_to_text: String, path_to_database: String, book_name: String) extends Initialize(path_to_text: String, path_to_database: String){
    ...
    def export_words: Unit = {
        ...
        try{
            Export = new PrintWriter(new File("src/main/scala/files/spreadsheets/" + book_name + "-words.csv"))
    ...
    def export_characters: Unit = {
        ...
        try{
            Export = new PrintWriter(new File("src/main/scala/files/spreadsheets/" + book_name + "-characters.csv"))
            var sb: StringBuilder = new StringBuilder()
    ...
    def export_data: Unit = {
        ...
        try{
            Export = new PrintWriter(new File("src/main/scala/files/spreadsheets/documents.csv"))
            var sb: StringBuilder = new StringBuilder()
            ...
            rt = conn.prepareStatement(query)
            var rs = rt.executeQuery()
            sb.append("book, num_words, num_char, avg_char_word, longest_word, length_25\r\n")
            while(rs.next()){
